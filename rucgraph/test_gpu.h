...
//iteration_times = inquire.size();
	cout << "inquire size " << inquire.size() << endl;

	cout << "read complete" << endl;
	unordered_map<int, int> vertexID_old_to_new;
	V = instance_graph.hash_of_vectors.size();
	for (int mm = 0; mm < V; mm++)
	{
		vertexID_old_to_new[mm] = mm;
	}
	for (int mm = V; mm < generated_group_graph.hash_of_vectors.size(); mm++)
	{
		vertexID_old_to_new[mm] = mm;
	}
	...
	cout << "group graph size " << v_generated_group_graph.size() << endl;
	...
	cout << "input graph size " <<  v_instance_graph.size() << endl;
	...
	for (int i = 2; i < iteration_times; i++)
	{
		...
		{
			//cout << inquire[i][j] << endl;
			generated_group_vertices.insert(inquire[i][j]);
		}
...
#pragma omp parallel for
		...
		if (1)
		{
			...
			if (!this_is_a_feasible_solution_gpu(solutions[0], generated_group_graph, generated_group_vertices))
			{
				cout << "Error: graph_v_of_v_idealID_DPBF_only_ec is not feasible!" << endl;
	...
	if (solution_cost_GPU_sum != 2*solution_cost_PrunedDPPlusPlus_sum)
	{
	...
	cout << "time_CPU_avg=" << time_PrunedDPPlusPlus_avg << "s" << endl;
}