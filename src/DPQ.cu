...
int *visit, *queue_size, *tree_cost, *new_queue_size, *best;
...
__global__ void Relax(queue_element *Queue_dev, int *queue_size, queue_element *new_queue_device, int *new_queue_size, int *sets_IDs, int *sets_IDS_pointer, int *edge, int *edge_cost, int *pointer, size_t pitch_node, size_t pitch_int, node *tree, int *tree_cost, int width, int inf, int *best, int full)
{
...
graph_hash_of_mixed_weighted DPBF_GPU(CSR_graph &graph, std::unordered_set<int> &cumpulsory_group_vertices, graph_v_of_v_idealID &group_graph, graph_v_of_v_idealID &input_graph)
{
	...
	cudaMallocManaged((void **)&queue_device, problem_size * sizeof(int));
	cudaMallocManaged((void **)&new_queue_device, problem_size * sizeof(int));
...
/* 		for (size_t i = 0; i < *queue_size; i++)
		...
		Relax<<<(*queue_size + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK, THREAD_PER_BLOCK>>>(queue_device, queue_size, new_queue_device, new_queue_size, non_overlapped_group_sets_IDs_gpu,
																							 non_overlapped_group_sets_IDs_pointer_device, all_edge, edge_cost, all_pointer, pitch_node, pitch_int, tree, tree_cost, width, inf, best, group_sets_ID_range);
